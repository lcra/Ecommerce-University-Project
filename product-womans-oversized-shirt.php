...
<body>
    ...
    <?php
        $itemsSQL = "SELECT item_id, name, gender, charSize, price, category, quantity, image, alt, description FROM items WHERE item_id = 11";
        $result = mysqli_query($conn, $itemsSQL);
        
        if ($row = mysqli_fetch_array($result)){?>
            <div class="content">
                <div class="image-collage">
                <img class="product-image" src="<?php echo $row['image']; ?>" alt="<?php echo $row['alt']; ?>">
                </div>
                <div class="product-info">
                    ...
                    <form method="post" action="product-womans-oversized-shirt.php?item_id=<?=$row['item_id']?>">
                        <input type="hidden" name="name" value="<?=$row['name'] ?>">
                        <input type="hidden" name="price" value="<?=$row['price'] ?>">
                        <input type="hidden" name="quantity" value="1">
                        <input type="radio" name="charSize" value="S" id="S" required>
